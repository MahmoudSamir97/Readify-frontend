...
const AllBooks = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    let url = selectedCategory === "All" ? "http://localhost:4000/book/AllBook" : `http://localhost:4000/book/getAllBookInCategory/${selectedCategory}`;
    axios.get(url)
      .then((response) => {
        setAllBooks(response.data.data.allBooks);
        setLoading(false);
      })
      .catch((error) => {
